...
/*In ra danh sách (mã học viên, họ tên, ngày sinh, mã lớp) lớp trưởng của các lớp.*/
	SELECT 
		...
		s.MALOP
	FROM HOCVIEN s
	JOIN LOP c ON c.TRGLOP = s.MAHV;

/*In ra bảng điểm khi thi (mã học viên, họ tên, lần thi, điểm số) môn CTRR của lớp “K12”, sắp 
xếp theo tên, họ học viên. */
	SELECT 
		s.MAHV,
		s.HO,
		s.TEN,
	...
	JOIN KETQUATHI r
	ON s.MAHV = r.MAHV
	WHERE MAMH LIKE 'CTRR' AND MALOP = 'K12'
	ORDER BY TEN, HO;
...
lần thứ nhất đã đạt. */
	...
	JOIN HOCVIEN s
	ON s.MAHV = r.MAHV 
	GROUP BY r.MAHV, s.HO, s.TEN, r.MAMH, KQUA
...
lần thi 1).*/
	...
	FROM (
		SELECT r1.MAHV, r1.LANTHI, s1.HO, s1.TEN
		FROM HOCVIEN s1
		JOIN KETQUATHI r1 
		ON s1.MAHV = r1.MAHV 
		WHERE s1.MALOP LIKE 'K%' AND r1.MAMH = 'CTRR' AND KQUA = 'Khong Dat'
	) A
	JOIN(
		SELECT MAHV, MAX(LANTHI) LANTHIMAX
		FROM KETQUATHI 
		WHERE MAHV LIKE 'K%' AND MAMH = 'CTRR' 
		GROUP BY MAHV, MAMH
	) B
	ON A.MAHV = B.MAHV
